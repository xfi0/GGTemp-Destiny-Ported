        ...
        public static List<(Color color, string name)> colorCycle = new List<(Color, string)>
{
    ...
    (new Color(176f / 255f, 153f / 255f, 128f / 255f), "maincolor")
        ...
        {
            if (EasyInputs.GetGripButtonFloat(EasyHand.RightHand) > 0.1f)
            {
                if (Physics.Raycast(GorillaLocomotion.Player.Instance.rightHandTransform.position, -GorillaLocomotion.Player.Instance.rightHandTransform.up, out var hitinfo))
                {
            ...
            if (GunSphere != null && (EasyInputs.GetGripButtonFloat(EasyHand.RightHand) <= 0.1f))
            {
