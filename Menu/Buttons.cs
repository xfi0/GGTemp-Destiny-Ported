    ...
    internal class Buttons
    {
        public static ButtonInfo[][] buttons = new ButtonInfo[][]
        {
            ...
            new ButtonInfo[] { // GunLib
                ...
                new ButtonInfo { buttonText = $"Smoothness: {(mods.num == 5f ? "Very Fast" : mods.num == 10f ? "Normal" : "Super Smooth")}", method = () => { mods.GunSmoothNess(); foreach (var category in Buttons.buttons) foreach (var button in category) if (button.buttonText.StartsWith("Smoothness")) button.buttonText = $"Smoothness: {(mods.num == 5f ? "Super Smooth" : mods.num == 10f ? "Normal" : "No Smooth")}"; }, isTogglable = false, toolTip = "Changes gun smoothness." }, new ButtonInfo{ buttonText = $"Gun Color: { mods.currentGunColor.name}", method = () => { mods.CycleGunColor(); foreach (var category in Buttons.buttons) {foreach (var button in category) {if (button.buttonText.StartsWith("Gun Color")){ button.buttonText = $"Gun Color: {mods.currentGunColor.name}";} } } }, isTogglable = false, toolTip = "Cycles through gun colors."},
