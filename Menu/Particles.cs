...
namespace GGTemps.Menu
{
    class Particles
    {
        ...
        public static void CreateBlackHole()
        {
            ...
            {
                Vector3 directionToBlackHole = blackHolePosition - GorillaLocomotion.Player.Instance.bodyCollider.transform.position;
        ...
        {
            if (EasyInputs.GetPrimaryButtonDown(EasyHand.RightHand) && !isIceSpearCast)
            {
                isIceSpearCast = true;
                Vector3 spellStartPosition = GorillaTagger.Instance.rightHandTransform.position;
                Quaternion spellStartRotation = GorillaTagger.Instance.rightHandTransform.rotation;
        ...
        {
            if (EasyInputs.GetPrimaryButtonDown(EasyHand.RightHand) && !isFireballCast)
            {
                isFireballCast = true;
                Vector3 spellStartPosition = GorillaTagger.Instance.rightHandTransform.position;
                Quaternion spellStartRotation = GorillaTagger.Instance.rightHandTransform.rotation;
        ...
        {
            if (EasyInputs.GetPrimaryButtonDown(EasyHand.RightHand) && !hasCastLightning)
            {
        ...
        {
            if (EasyInputs.GetPrimaryButtonDown(EasyHand.RightHand) && !hasCastVoidRift)
            {
        ...
        {
            if (EasyInputs.GetPrimaryButtonDown(EasyHand.RightHand) && !hasCastFrostOrb)
            {
